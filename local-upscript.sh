...
LOCALV4_PREFIXES="145.220.8.0/21 10.13.37.0/24"
...
echo "Creating the downlink interface..."
vconfig add ${BOND_INTERFACE} ${DNLINK_VLAN}
ip link set ${BOND_INTERFACE}.${DNLINK_VLAN} up name vlan-dnlink
ip -4 addr add ${DNLINKV4_ADDRESS}/${DNLINKV4_PREFIXLEN} dev vlan-dnlink
for prefix in ${LOCALV4_PREFIXES};do
	ip -4 route add ${prefix} via ${DNLINKV4_LOCAL_GATEWAY}
done
ip -6 addr add ${DNLINKV6_ADDRESS}/${DNLINKV6_PREFIXLEN} dev vlan-dnlink
for prefix in ${LOCALV6_PREFIXES};do
	ip -6 route add ${prefix} via ${DNLINKV6_LOCAL_GATEWAY}
done
